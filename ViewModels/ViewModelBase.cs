        ...
        public ICommand DoneSelecting
        {
            get
            {
                _doneSelecting = new RelayCommand((x) =>
                {
                    ...
                    Window win;
                    if (_ItemSearchView == null) { MessageBox.Show("尚未選取圖檔"); return; }
                    ...
                    if (ClsSynchronizer.IsActiveSubDialogView == true)
                    {
                        ...
                        //if (ClsSynchronizer.SubDialogReturnValue == "") { MessageBox.Show("尚未選取圖檔"); return; }
                        if (String.IsNullOrWhiteSpace(ClsSynchronizer.SubDialogReturnValue)) { MessageBox.Show("尚未選取圖檔"); return; }
                    ...
                    else
                    {
                        ...
                        //if (ClsSynchronizer.DialogReturnValue == "") { MessageBox.Show("尚未選取圖檔"); return; }
                        if (String.IsNullOrWhiteSpace(ClsSynchronizer.DialogReturnValue)) { MessageBox.Show("尚未選取圖檔"); return; }
        ...
        private bool CheckItemSearchView()
        {
            ...
            if (_ItemSearchView != null)
            {
                ...
                if (String.IsNullOrWhiteSpace(txtSelectedItemId.Text) || String.IsNullOrWhiteSpace(ClsSynchronizer.VmDirectory))
                {
                    ...
                    //if (ClsSynchronizer.VmDirectory == "") MessageBox.Show("尚未選取儲存路徑");
                    if (String.IsNullOrWhiteSpace(ClsSynchronizer.VmDirectory)) MessageBox.Show("尚未選取儲存路徑");
                    //if (txtSelectedItemId.Text == "") MessageBox.Show("尚未選取圖檔");
                    if (String.IsNullOrWhiteSpace(txtSelectedItemId.Text)) MessageBox.Show("尚未選取圖檔");
                    StartStopWait(false);
                    return false ;
                }
        ...
        public ICommand SettingOK
        {
            get
            {
                _settingOK = new RelayCommand((x) =>
                {
                    ...
                    ResourceDictionary resourceDictionary = new ResourceDictionary();
                    resourceDictionary.Source = new Uri($"pack://application:,,,/BCS.CADs.Synchronization;Component/Lang/{SelectedLang.Value}.xaml");
